...
const ShareIndicationBar = props => {
    ...
    getAntdItem(
      ...
      [
        getAntdItem(
          "Fit to Window",
          "view|fit",
          "view|fit".endsWith(viewType) && <CheckOutlined />
        ),
        getAntdItem(
          "Orignial Size",
          "view|original",
          "view|original".endsWith(viewType) && <CheckOutlined />
        )
      ],
      "group"
    )
  ...
  if (
    mediaStream?.isRemoteControlEnabled() &&
    mediaStream?.isTargetShareSupportRemoteControl(activeSharingId)
  ) {
    menuItems.push(getAntdItem("", "d1", undefined, undefined, "divider"))
    menuItems.push(
      getAntdItem(
        isControllingUser ? "Give up Remote Control" : "Request Remote Control",
        "remote control"
      )
    )
  }
  if (activeUser?.bShareAudioOn) {
    ...
    menuItems.push(
      getAntdItem(
        `${mediaStream?.isOthersShareAudioMutedLocally(activeSharingId)
          ? "Unmute"
          : "Mute"
        } ${activeUser.displayName}'s Computer Audio`,
        "share audio"
      )
    )
  }
      ...
      getAntdItem(
        ...
        (sharUserList ?? []).map(user =>
          getAntdItem(
            user.displayName,
            `share|${user.userId}`,
            activeSharingId === user.userId && <CheckOutlined />
          )
        ),
        "group"
      )
  ...
  const onMenuClick = payload => {
    ...
    if (key.startsWith("view|")) {
      ...
      if (mediaStream?.isOthersShareAudioMutedLocally(activeSharingId)) {
        ...
        setMutedShareAudioList(
          mutedShareAudioList.filter(u => u !== activeSharingId)
        )
      } else {
  ...
  return (
    <Draggable nodeRef={draggableRef} axis="x" bounds="parent">
      <div className="share-indication-bar" ref={draggableRef}>
        <p
          className={classNames("share-indication-tip", {
            "share-indication-in-control": isControllingUser
          })}
        >
          {activeUser?.bShareAudioOn && (
            <IconFont
              type={
                mutedShareAudioList.includes(activeSharingId)
                  ? "icon-audio-off"
                  : "icon-audio-on"
              }
            />
          )}
          {`You are ${isControllingUser ? "controlling" : "viewing"} ${activeUser?.displayName
            }'s screen`}
        </p>
        <Dropdown
