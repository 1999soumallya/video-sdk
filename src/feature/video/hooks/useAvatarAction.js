  ...
  switch (action.type) {
    case "toggle-local-volume": {
      ...
      if (userItem !== undefined) {
        draft[`${userId}`].localVolumeAdjust.toggled = !draft[`${userId}`]
          .localVolumeAdjust.toggled
      }
      break
    }
    case "toggle-far-end-camera-control": {
      ...
      const userItem = draft[`${userId}`]
      if (userItem !== undefined) {
        draft[`${userId}`].farEndCameraControl.toggled = !draft[`${userId}`]
          .farEndCameraControl.toggled
      }
      break
    }
    case "update-avatar-action-enable": {
      ...
      if (Object.hasOwn(draft, `${userId}`)) {
        ...
        element.videoResolutionAdjust.enabled =
          isUnifiedRender && bVideoOn && currentUserId !== userId
      } else {
        const state = {
          localVolumeAdjust: {
            toggled: false,
    ...
    case "toggle-video-resolution-adjust": {
      ...
      const userItem = draft[`${userId}`]
      if (userItem !== undefined) {
        draft[`${userId}`].videoResolutionAdjust.toggled = !draft[`${userId}`]
